...
public final class JSIOnCameraChangeListener {
    ...
    @JavascriptInterface
    public void onCameraChange(final double lat, final double lng, final float zoom) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                listener.onCameraChange(CameraPosition.fromLatLngZoom(new LatLng(lat, lng), zoom));
            }
        });
    }
}
